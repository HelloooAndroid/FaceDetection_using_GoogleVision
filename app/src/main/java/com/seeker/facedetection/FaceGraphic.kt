...
import com.seeker.facedetection.Camera.FaceTrackingListener
...
internal class FaceGraphic(overlay: GraphicOverlay) : GraphicOverlay.Graphic(overlay) {
    ...
    override fun draw(canvas: Canvas) {
        ...
        canvas.drawText(
            "left eye: " + String.format("%.2f", face.getIsRightEyeOpenProbability()),
            (x + ID_X_OFFSET) / 2 ,
            y - ID_Y_OFFSET * 2 + 60,
            mIdPaint
        );
        canvas.drawText(
            "right eye: " + String.format("%.2f", face.isLeftEyeOpenProbability),
            x - ID_X_OFFSET * 2,
            y - ID_Y_OFFSET * 2 + 60,
            mIdPaint
        )
        ...
        /*canvas.drawText(
            "Euler Y: " + String.format("%.2f", EulerY),
            x - ID_X_OFFSET,
            y - ID_Y_OFFSET,
            mIdPaint
        )
        canvas.drawText(
            "Euler Z: " + String.format("%.2f", EulerZ),
            x + ID_X_OFFSET * 2,
            y + ID_Y_OFFSET * 2,
            mIdPaint
        )*/
        ...
        for (landmark in face.landmarks) {
            val cx = (landmark.position.x * 2) as Int
            val cy = (landmark.position.y * 2) as Int
            canvas.drawCircle(cx.toFloat(), cy.toFloat(), 10f, mPaint)
        }
